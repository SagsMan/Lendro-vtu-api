...
const VerifyFund = async ({refno,total,setWalletData,setMaxCUPoint,setTransactions})=>{
      const r = await apiFetch("/api/deposit.php", {category:"deposited",status:"processed",refno:refno,total:total});
      try{
              if (r.status === "success" && r.data) {
                ...
                alert(`deposit of ${formatCurrency(r.data?.amount)} failed.`);
              }
      }catch(e){
...
const Buy = async ({category,data,setWalletData,setMaxCUPoint,setBusy,setPopupOpen,setTransactions})=>{
    ...
    if(category === "deposit"){
      ...
      url = "/api/deposit.php";
      body = {title:"Deposit",category,amount:data?.amount,fee:data?.fee,total:data?.total,status:"pending"};
      ////////////////////////
      ...
      body = {catTitle:data?.catTitle,category,serviceid:data?.serviceid,phone:data?.myphone,amount:data?.amount};
    ...
    try{
      ...
      if (res?.status === "success" && res.data) {
        ...
        if(category === "airtime"){ 
          alert(`${formatCurrency(data?.amount)} ${data?.category} airtime successfully purchased. You'll receive it shortly`);

        }else if(category === "data"){ 
          alert(`${formatCurrency(data?.amount)} data successfully purchased. You'll receive it shortly`);

        }else if(category === "deposit"){ 
          let amt = res.data?.amount || data?.total;
          let email = res.data?.email || "favoursdot@gmail.com";
        ...
        const m = res?.message || res.data?.message; console.log(res?.message,res?.data,res?.param);
...
const PopUpModal = ({ popupOpen, setPopupOpen, onClose, onSubmit, wallet, services,setBusy,setWalletData,setMaxCUPoint,setTransactions }) => {
    ...
    const data = popupOpen?.data; //data:{dwat:"buyairtime",biller:aName,code:aCode,maxPrice:aMaxPrice}
    if(!data) return null;
  
    return e("div", null, //React.Fragment...
    /* Backdrop */
...
const billerForm = ({data,services,wallet,setBusy,setWalletData,setMaxCUPoint, setPopupOpen,setTransactions})=>{
  ...
  let catTitle = null;
  let category = (dwat === "buyairtime") ? "airtime":"data";
  
  if(dwat === "buyairtime"){ catTitle = "Airtime"; category = "airtime"; }
  else if(dwat === "buydata"){catTitle = "Data";  category = "data";}
...
return e("div",{},
          e("h3", { key: "title",  className: "text-lg font-semibold mb-3" }, `Buy${(data?.biller)?" "+flwNames[data?.biller]:""} ${catTitle}`),
          //e("span",{},"Receiver's phone number"),
          e("div", { key: "rphone", className: "flex items-center w-full mb-2" },
              ...
              e("input",{type:"tel", placeholder:"8012345678",name:"rphone",maxLength:11, className:"w-full px-3 py-2 rounded-r-lg bg-white border border-gray-400 border-l-0 focus:outline-none", value:myphone || "",onChange: (e) => setMyPhone(e.target.value.replace(/\D/g, "").replace(/^0+/, "").slice(0, 10)) }),
          ),
          e("div", { key: "ramount", className: "flex items-center w-full mb-1" },
              ...
              e("input",{type:"number",step:1,min:100, placeholder:"Enter amount",name:"ramount",className:"w-full px-3 py-2 rounded-r-lg bg-white border border-gray-400 border-l-0 focus:outline-none",value: amount, onChange: (e) => setAmount(e.target.value)}),
          ),
          e("div", { className: "mb-3 text-sm text-gray-500"}, `Allowed limit: ${formatCurrency(100)} - ${formatCurrency(data?.maxPrice || 50000)}`),

          e("button", { key: "buy", type: "button",  className: "w-full bg-indigo-600 text-white px-4 py-3 rounded-lg font-semibold active:scale-95 transition mb-5",onClick:(e)=>Buy({category,data:{catTitle,serviceid:data?.code,myphone,amount,balance},setWalletData,setMaxCUPoint,setBusy,setPopupOpen}) },`Pay${(amount > 0) ? " "+formatCurrency(amount):""}`),

          //Predefined prices for airtime
          e("div", { key: "preamounts", className: "flex flex-wrap items-center gap-4 w-full mb-5" },
            ...
            (dwat === "buyairtime") && [100,200,300,400,500,700,1000,2000,3000].map((price,i) => {  
              return e("span", { key:i,className: "flex-[0_0_calc(33.333%-1rem)] px-3 py-2 bg-gray-500/20 rounded-lg text-lg text-center",onClick:()=>setAmount(price)}, `${formatCurrency(price,false)}`)
            }),
          ),
      )
};
...
const depositForm = ({data,wallet,setBusy,setWalletData,setMaxCUPoint,setPopupOpen,setTransactions})=>{
  const [amount, setAmount] = useState("");
  const [fee, setFee] = useState(0); // 2.5%, capped 2500
  ...
  return e("div",{},
          ...
          e("div", { key: "amount", className: "flex items-center w-full mb-1" },
              e("span", { className: "px-3 py-2 bg-gray-500/20 border border-gray-400 border-r-0 rounded-l-lg text-md"}, "₦"),
              e("input",{type:"number",step:1,min:100, placeholder:"Enter amount",name:"ramount",className:"w-full px-3 py-2 rounded-r-lg bg-white border border-gray-400 border-l-0 focus:outline-none",value: amount, onChange: (e) => calcAmount(e.target.value)}),
          ),
          ...
          (amount > 0) && e("div", { className: "mb-3 text-sm text-gray-900 bg-yellow-100 p-3 rounded-md text-center border-2"}, 
            e("span", { className: "mb-1 text-gray-900"}, `Pay ${formatCurrency(total)} to deposit (${formatCurrency(fee)} fee included).`
          ),
          ),

          e("button", { key: "buy", type: "button",  className: "w-full bg-indigo-600 text-white px-4 py-3 rounded-lg font-semibold active:scale-95 transition mb-5",onClick:(e)=>Buy({category:"deposit",data:{amount,fee,total},setWalletData,setMaxCUPoint,setBusy,setPopupOpen,setTransactions}) },`Deposit`),
          //Predefined prices for airtime
          e("div", { key: "preamounts", className: "flex flex-wrap items-center gap-4 w-full mb-5" },
            
            [100,200,500,1000,2000,3000,4000,5000,10000].map((price,i) => {  
              return e("span", { key:i,className: "flex-[0_0_calc(33.333%-1rem)] px-3 py-2 bg-gray-500/20 rounded-lg text-lg text-center",onClick:()=>calcAmount(price)}, `${formatCurrency(price,false)}`)
            }),

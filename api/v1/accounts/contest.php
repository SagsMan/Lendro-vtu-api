...
$stmt = $db->prepare("SELECT users.fullname,wallets.id,wallets.plan,wallets.plan,wallets.userid,wallets.usage_recent,wallets.upoint FROM wallets INNER JOIN users ON wallets.userid = users.id WHERE wallets.usage_recent > 0 AND wallets.repayscore >= 0 ORDER BY wallets.usage_recent DESC LIMIT 20");
$stmt->execute([]);
